; Program to calculate geometric information for each square
; pyramid in a series of square pyramids

; The program calculates the total surface area and
; volume of each pyramid

; Once the values are computed, the program finds the minimum,
; maximum, sum, and average for the total surface area and
; volumes of the pyramids

; Formulas:
;   totalSurfaceAreas(n) = aSides(n)^2+ 2 * aSides(n) * sSides(n)
;   volumes(n) = (aSides(n) ^ 2 * heights(n)) / 3
; check out the following link for the formulas and what sides a, s, and height are:
; https://www.calculatorsoup.com/calculators/geometry-solids/pyramid.php
...
totalAreas    resd    50      ; resd means reserve a d (doubleword)
volumes       resd    50      ; and the 50 means how many to reserve
...
;   totalAreas(n) = aSides(n) * (2*aSides(n)*sSides(n))
  ...
  movzx   r9d, word [sSides+rsi*2]    ; sSides[i] (*2 because these are words)
  mov     eax, r8d
  mul     dword [ddTwo]
  mul     r9d
  mul     r8d
...
;   volumes(n) = (aSides(n)^2 * heights(n)) / 3
  ...
  mul     eax
  mul     dword [heights+rsi*4]      ; (rsi*4 for double words)
  div     dword [ddThree]
...
;   find min, max, sum, average for total areas and volumes
